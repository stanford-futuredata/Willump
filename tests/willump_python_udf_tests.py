...
class WillumpPythonUDFTests(unittest.TestCase):
    ...
    def test_basic_udf(self):
        print("\ntest_basic_udf")
        sample_python: str = inspect.getsource(sample_basic_udf)
        sample_string: str = "cat dog \n house. "
        self.set_typing_map(sample_python, "sample_basic_udf", [sample_string])
        ...
        exec(compile(compiled_functiondef, filename="<ast>", mode="exec"), globals(),
             local_namespace)
        weld_output = local_namespace["sample_basic_udf"](sample_string)
