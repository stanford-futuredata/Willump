...
if __name__ == "__main__":
    ...
    c = np.ones(1) * 299.792458
    # Add extra features:
    num_rows = len(df.index)
    add_features(df.iloc[0:1].copy())
    add_features(df.iloc[0:1].copy())
    add_features(df.iloc[0:1].copy())
    copied_slices = []
    for i in range(num_rows):
        copied_slices.append(df.iloc[i:i+1].copy())
    print("Created %d copied slices" % num_rows)
    t0 = time.time()
    for copied_slice in copied_slices:
        aug_df = add_features(copied_slice)
