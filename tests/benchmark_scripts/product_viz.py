...
def vectorizer_transform_unopt(title_vect, input_df, color_vect, brand_vect):
    np_input = list(input_df.values)
    transformed_result = title_vect.transform(np_input)
    color_result = color_vect.transform(np_input)
    brand_result = brand_vect.transform(np_input)
...
def with_willump():
    for entry in tqdm(entry_list, position=1, desc="With Willump"):
        vectorizer_transform(title_vectorizer, entry, color_vectorizer, brand_vectorizer)


def sans_willump():
    for entry in tqdm(entry_list, desc="Sans Willump"):
        vectorizer_transform_unopt(title_vectorizer, entry, color_vectorizer, brand_vectorizer)


pw = Process(target=with_willump)
ps = Process(target=sans_willump)

print("Starting point speed comparison!")
