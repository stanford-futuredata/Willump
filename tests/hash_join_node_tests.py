    ...
    def test_basic_hash_join(self):
        ...
        hash_join_node: WillumpHashJoinNode = \
            WillumpHashJoinNode(input_node, "output", "join_column", right_table, aux_data,
                                size_left_df=3, join_col_left_index=0)
        ...
        type_map = {"__willump_arg0": WeldStruct([WeldVec(WeldLong()), WeldVec(WeldLong()), WeldVec(WeldLong())]),
                    "__willump_retval0": WeldStruct([WeldVec(WeldLong()), WeldVec(WeldLong()), WeldVec(WeldLong()), WeldVec((WeldDouble())), WeldVec((WeldDouble()))])}
