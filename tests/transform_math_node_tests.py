...
class BasicEvaluationTests(unittest.TestCase):
    ...
    def test_add_vars(self):
        print("\ntest_add_vars")
        in_node: wgin.WillumpInputNode = wgin.WillumpInputNode()
        index_op_one = tmn.MathOperation("+", False, first_input_index=0, second_input_index=0,
                                         store_result_var_name="two")
        index_op_two = tmn.MathOperation("+", True, first_input_index=1, second_input_index=2,
                                         store_result_var_name="five")
        index_op_three = tmn.MathOperation("+", True, first_input_var="five",
                                           second_input_literal=5.)
        index_op_four = tmn.MathOperation("+", True, first_input_var="two", second_input_var="two")
        add_node: wgn.WillumpGraphNode = \
            tmn.TransformMathNode(in_node, [index_op_one, index_op_two, index_op_three,
                                            index_op_four])
        out_node: wgon.WillumpOutputNode = wgon.WillumpOutputNode(add_node)
    ...
    def test_sqrt(self):
        print("\ntest_sqrt")
        in_node: wgin.WillumpInputNode = wgin.WillumpInputNode()
        index_op_one = tmn.MathOperation("sqrt", False, first_input_literal=4.0,
                                         store_result_var_name="two")
        index_op_two = tmn.MathOperation("sqrt", True, first_input_index=0)
        index_op_three = tmn.MathOperation("+", True, first_input_literal=5.0, second_input_var="two")
        add_node: wgn.WillumpGraphNode = \
            tmn.TransformMathNode(in_node, [index_op_one, index_op_two, index_op_three])
        out_node: wgon.WillumpOutputNode = wgon.WillumpOutputNode(add_node)
        graph: wg.WillumpGraph = wg.WillumpGraph([in_node], out_node)
