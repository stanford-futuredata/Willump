...
class WillumpPredictProbaNode(WillumpPythonNode, WillumpModelNode):
    """
    ...
    def __init__(self, model_name: str, x_name: str,
        ...
        predict_statement = "%s = willump_predict_proba_function(%s, %s)" % (strip_linenos_from_var(output_name),
                                                                       model_name, strip_linenos_from_var(x_name))
        predict_ast: ast.Module = ast.parse(predict_statement, "exec").body[0]
        super(WillumpPredictProbaNode, self).__init__(python_ast=predict_ast, input_names=[x_name],
    ...
    def __repr__(self):
        return "Willump Predict Proba Python node with inputs %s\n outputs %s\n and code %s\n" % \
