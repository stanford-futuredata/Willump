...
def generate_cpp_driver(file_version: int, type_map: Mapping[str, WeldType]) -> str:
    ...
    with open(os.path.join(willump_home, "cppextensions", "weld_llvm_caller.cpp"), "r") as infile:
        ...
        buffer = buffer.replace("WELD_INPUT_TYPE_0", weld_type_to_str(input_type))
        buffer = buffer.replace("WELD_OUTPUT_TYPE", weld_type_to_str(output_type))
...
def weld_type_to_str(wtype: WeldType) -> str:
    """
